...
/**
 * Lifted from LangX (src/lib/runtime/llm/worker.ts, same author) — the Web
 * Worker that owns the transformers.js pipeline. Kept intact: Cache Storage
 * persistence, per-file progress events, `enable_thinking:false`, chat-template
 * tools pass-through, and the serialized one-inference-at-a-time work queue.
 */
import {
	pipeline,
	env,
